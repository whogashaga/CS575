...
#ifndef FILE_NAME
#define FILE_NAME		"mult.cl"
#endif
const char* CL_FILE_NAME = { FILE_NAME };
...
main( int argc, char *argv[ ] )
{
	...
	int tmp ;
	if (CL_FILE_NAME == "Mult.cl")
		tmp = 2;
	else 
		tmp = 3;

	status = clSetKernelArg( kernel, tmp, sizeof(cl_mem), &dD );
	if( status != CL_SUCCESS )
	...
	fprintf( stderr, "%5d, %8d, %4d, %10d, %10.3lf,MegaMults/Sec\n",
		NMB, NUM_ELEMENTS, LOCAL_SIZE, NUM_WORK_GROUPS, (double)NUM_ELEMENTS/(time1-time0)/1000000. );
